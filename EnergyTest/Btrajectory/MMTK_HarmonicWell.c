/* C routines for HarmonicWellFF.py */
...
static void
HarmonicWell_evaluator(PyFFEnergyTermObject *self,
     ...
     */
{
  ...
  double x = self->param[1];  /* reference point x */
  double y = self->param[2];  /* reference point y */
  double z = self->param[3];  /* reference point z */
  double k = self->param[4];  /* force constant */
...
/* The next function is meant to be called from Python. It creates the
   ...
   module, HarmonicWellFF.py. */
static PyObject *
HarmonicWellTerm(PyObject *dummy, PyObject *args)
{
  ...
  self->eval_func = HarmonicWell_evaluator;
  /* The name of the energy term object. */
  self->evaluator_name = "HarmonicWell";
  /* The names of the individual energy terms - just one here. */
  self->term_names[0] = allocstring("HarmonicWell");
...
static PyMethodDef functions[] = {
  {"HarmonicWellTerm", HarmonicWellTerm, 1},
  {NULL, NULL}		/* sentinel */
};
...
DL_EXPORT(void)
initMMTK_HarmonicWell(void)
{
  PyObject *m;

  /* Create the module and add the functions. */
  m = Py_InitModule("MMTK_HarmonicWell", functions);
  ...
  /* Check for errors. */
  if (PyErr_Occurred())
    Py_FatalError("can't initialize module MMTK_HarmonicWell");
}
