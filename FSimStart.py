...
integrator(steps = 3000, actions = [ TrajectoryOutput(None,('configuration','time'), 0, None, 100)] )  # relates to the default_options = {'first_step': 0...} section of the main code.

RunSteps = int(numsteps)*Units.fs/dt
SkipSteps = 1.0*Units.fs/dt

trajectory = Trajectory(universe, outdir+"N"+str(nmolecules)+"H20T"+str(temperature)+"P"+str(P)+"R"+str(lattice_spacing)+"FilEFVersion"+str(numsteps)+"Steps"+label+".nc", "w", "A simple test case")
...
cosfile1 = open("cosfile1-"+str(nmolecules)+"H20T"+str(temperature)+"P"+str(P)+"FileFVersioN"+str(numsteps)+"Steps"+label,"w")
cosfile2 = open("cosfile2-"+str(nmolecules)+"H20T"+str(temperature)+"P"+str(P)+"FileFVersioN"+str(numsteps)+"Steps"+label,"w")
dihedral = open("dihedral-"+str(nmolecules)+"H20T"+str(temperature)+"P"+str(P)+"FileFVersioN"+str(numsteps)+"Steps"+label,"w")
...
for step in trajectory:
    ...
    for i in range(P):
        for a in range(len(universe.objectList()) - 1):         # loops over all molecules
            ...
            normdiff = sqrt(dot(diffr,diffr))
            norm1 = sqrt(dot(mu1,mu1))
            norm2 = sqrt(dot (mu2, mu2))
            ...
            cost1 = dot(diffr,mu1)
            cost2 = dot(diffr,mu2)
            dihedralangle = dot(mu1, mu2)

            cosfile1.write(str(cost1)+"\n")
            cosfile2.write(str(cost2)+"\n")
            dihedral.write(str(dihedralangle)+"\n")
