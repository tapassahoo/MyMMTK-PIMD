...
*/
  ...
  charge = 1.8546*3.33564e-30; //Coulomb
  ...
  lengthmu1 = sqrt( mu1[0]*mu1[0] + mu1[1]*mu1[1] + mu1[2]*mu1[2] );

  mu2[0] = x6 - (x5 + x4)/2;
  mu2[1] = y6 - (y5 + y4)/2;
  mu2[2] = z6 - (z5 + z4)/2;

  lengthmu2 = sqrt( mu2[0]*mu2[0] + mu2[1]*mu2[1] + mu2[2]*mu2[2] );

  e_1[0] = mu1[0]/lengthmu1;
  e_1[1] = mu1[1]/lengthmu1;
  e_1[2] = mu1[2]/lengthmu1;

  e_2[0] = mu2[0]/lengthmu2;
  e_2[1] = mu2[1]/lengthmu2;
  e_2[2] = mu2[2]/lengthmu2;
  ...
  theta1 = acos( e_1[0]*e_r[0] + e_1[1]*e_r[1] + e_1[2]*e_r[2] );
  theta2 = acos( e_2[0]*e_r[0] + e_2[1]*e_r[1] + e_2[2]*e_r[2] );
  phi = theta1 - theta2;
  ...
  double predv=(mu*mu/(4.0*M_PI*r*r*r*8.854187817e-12))*(6.022140857e23)/1000.0;

  e = predv*(sin(theta1)*sin(theta2)*cos(phi) - 2.0*cos(theta1)*cos(theta2));
